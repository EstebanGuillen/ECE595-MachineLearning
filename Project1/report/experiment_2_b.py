...
for restarts in range(0,Restarts):
	...
	for l in range(0,Iterations):
		...
		mu_list1.append(np.matrix.copy(mu))
		sigma_list1.append(np.matrix.copy(sigma))
		...
		if(total_iterations_em > 3
			and np.allclose(mu_list1[l],mu_list1[l-1],rtol=1e-06) 
			and np.allclose(sigma_list1[l],sigma_list1[l-1],rtol=1e-06) ):		
			break
	end = timer()
	...
	for l in range(0,Iterations):
		...
		mu_list2.append(np.matrix.copy(mu))
		sigma_list2.append(np.matrix.copy(sigma))
		...
		total_iterations_em = total_iterations_em + 1
			...
			#break
		if(total_iterations_em > 3
			and np.allclose(mu_list2[l],mu_list2[l-1],rtol=1e-06) 
			and np.allclose(sigma_list2[l],sigma_list2[l-1],rtol=1e-06) ):		
			break
	...
	for l in range(0,Iterations):
		...
		mu_list3.append(np.matrix.copy(mu))
		...
		total_iterations_kmeans = total_iterations_kmeans + 1
			...
			#break
		if(total_iterations_kmeans > 3
			and np.allclose(mu_list3[l],mu_list3[l-1],rtol=1e-06) ):		
			break
...
plt.clf()
vis.visualize_iterations_per_restart(iterations_for_each_restart_em, title='Iterations_per_Restart_with_Exit_Criteria (2) - EM', delay=5.0,save_plot=True)

plt.clf()
vis.visualize_iterations_per_restart(iterations_for_each_restart_kmeans_maha, title='Iterations_per_Restart_with_Exit_Criteria (2) - K-means (Maha)', delay=5.0, save_plot=True)

plt.clf()
vis.visualize_iterations_per_restart(iterations_for_each_restart_kmeans_eucl, title='Iterations_per_Restart_with_Exit_Criteria (2) - K-means (Eucl)', delay=5.0, save_plot=True)
