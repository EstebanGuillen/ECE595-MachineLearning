...
def visualize_optimal_K_em(K, title='Average_Log_Likelihood_vs_K', delay=15.0,save_plot=False):
	title_string = title
...
def visualize_optimal_K_kmeans(K, title='Average_Distance_vs_K_Size', measure='Average Distance', delay=15.0,save_plot=False):
	title_string = title
	plt.title(title_string)
	plt.xlabel('K')
	plt.ylabel(measure)
...
def visualize_iterations_per_restart(C, title='Iterations_per_Restart_with_Exit_Criteria', delay=5.0,save_plot=False):
	title_string = title
	plt.title(title_string)
	plt.xlabel('Restart Index')
	plt.ylabel('Number of Iterations')
	plt.bar(range(0,30), C)
	if save_plot:
		plt.savefig(title_string + '.png')
	plt.pause(delay)
